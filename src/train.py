"""Training entrypoint: scaled LSTM forecasting with inverse-transformed metrics, baselines, and visualizations."""
...
def train_and_evaluate(
    ...
    """Train a single LSTM model and produce metrics/plots."""
    # DataLoaders
    batch_size = 128
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        print(f"[{model_name}] Epoch {epoch:02d} | train_loss={avg_train:.4f} | val_loss={avg_val:.4f}")
    ...
    print(f"[{model_name}] Inverse-transformed y_val shape: {y_val_true.shape}, y_pred shape: {y_val_pred.shape}")
    print(f"[{model_name}] Inverse-transformed y_test shape: {y_test_true.shape}, y_pred shape: {y_test_pred.shape}")

    model_val_metrics = evaluate_forecasts(y_val_true, y_val_pred)
    model_test_metrics = evaluate_forecasts(y_test_true, y_test_pred)
    print(f"[{model_name}] Validation metrics (original scale):", model_val_metrics)
    print(f"[{model_name}] Test metrics (original scale):", model_test_metrics)

    # Per-horizon MAE
    horizons = np.arange(1, forecast_horizon + 1)
    mae_per_h = [np.mean(np.abs(y_test_true[:, h - 1] - y_test_pred[:, h - 1])) for h in horizons]
    pd.DataFrame({"horizon": horizons, "MAE": mae_per_h}).to_csv(results_dir / f"metrics_per_horizon_{model_name.lower()}.csv", index=False)
    ...
    mae_hour.to_csv(results_dir / f"mae_by_hour_{model_name.lower()}.csv")
    mae_dow.to_csv(results_dir / f"mae_by_dow_{model_name.lower()}.csv")
    ...
    _plot_24h(0, f"24h forecast sample ({model_name})", results_dir / f"plot_24h_sample_{model_name.lower()}.png")
    _plot_24h(len(y_test_true) // 2, f"24h forecast sample mid ({model_name})", results_dir / f"plot_24h_sample_mid_{model_name.lower()}.png")
    ...
    _plot_week(0, f"Week forecast start ({model_name})", results_dir / f"plot_week_start_{model_name.lower()}.png")
    _plot_week(len(y_test_true) // 2, f"Week forecast mid ({model_name})", results_dir / f"plot_week_mid_{model_name.lower()}.png")

    mae_hour.plot(kind="bar", figsize=(10, 4), title=f"MAE by hour of day ({model_name})")
    plt.ylabel("MAE")
    plt.tight_layout()
    plt.savefig(results_dir / f"mae_by_hour_{model_name.lower()}.png")
    plt.close()

    mae_dow.plot(kind="bar", figsize=(8, 4), title=f"MAE by day of week ({model_name})")
    plt.ylabel("MAE")
    plt.tight_layout()
    plt.savefig(results_dir / f"mae_by_dow_{model_name.lower()}.png")
    plt.close()

    joblib.dump(scaler_y, results_dir / f"scaler_y_{model_name.lower()}.pkl")
...
def main() -> None:
    ...
    scaler_y = StandardScaler()
    scaler_x.fit(train_df[feature_cols].to_numpy())
    scaler_y.fit(train_df[[TARGET_COL]])

    def _apply_scalers(df):
        df_scaled = df.copy()
        df_scaled[feature_cols] = scaler_x.transform(df[feature_cols].to_numpy())
    ...
    X_train, y_train = make_sequences(train_scaled, TARGET_COL, input_width=window, forecast_horizon=forecast_horizon, feature_cols=feature_cols)
    X_val, y_val = make_sequences(val_scaled, TARGET_COL, input_width=window, forecast_horizon=forecast_horizon, feature_cols=feature_cols)
    X_test, y_test = make_sequences(test_scaled, TARGET_COL, input_width=window, forecast_horizon=forecast_horizon, feature_cols=feature_cols)
    ...
    all_metrics = baseline_results.copy()
    lstm_metrics = train_and_evaluate(
        model_name="LSTM",
    ...
    pd.DataFrame(all_metrics).T.to_csv(results_dir / "metrics_global_all.csv")
